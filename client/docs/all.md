...
| **Persistence**  | Данные между сессиями      | Сервер + cookie-session |
...
5. Браузерное хранилище не используется для данных приложения; серверные данные запрашиваются через API и хранятся в кеше приложения.
